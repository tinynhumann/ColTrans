...
public class DecryptorALT3 {
    ...
    public DecryptorALT3(String secretPath, String knownPlaintext) throws IOException {
        ...
        testBlockLenghts = primefactor(secretTxt.length() - 1);
        //testBlockLenghts = Arrays.asList(2, 5);
    ...
    private List<Integer> findCols(char[][] blocks) {
        ...
        //List<int[]> assumedKeys = new ArrayList<>();
...
//                if(blocks[col].toString()){
                assumedKey[c] = known.indexOf(blocks[col][c]);
                if (assumedKey[c] >= 0 ){//&& assumedKey[c] < blockLength) {
                    matches++;
                } else {
                    matches--;
                }
            ...
            if (matches >= 0) {
            //    if (matches == 5) {
                //System.out.println(col + " " + Arrays.toString(assumedKey));
                interestingCols.add(col);
                //assumedKeys.add(assumedKey);
            }
            if(matches==5){
              System.out.println("\n\n"+Arrays.toString(assumedKey)+"\n\n");
            }
            //                String line = col + "\t" + matches + "\t" + Arrays.toString(blocks[col]);
//                if (interestingCols.contains(col)) {
//                    if (matches == 5) {
//                        String key = Arrays.toString(assumedKeys.get(assumedKeys.size() - 1));
//                        System.out.println((char) 27 + "[31m" + line + (char) 27 + "[0m" + "\t" + key);
//                    } else {
//                        System.out.println((char) 27 + "[31m" + line + (char) 27 + "[0m");
//                    }
//                } else {
        ...
        for (int c = 0; c < blockLenght; c++) {
            for (int col = 0; col < colLength; col++) {
                if (i < secretTxt.length() - 1) {
                    ...
                    //System.out.println(secretTxt.charAt(i));
                    i++;
                }
            }
        }
//        for (int col = 0; col < colLength; col++) {
//            System.out.println(col + " " + Arrays.toString(originalBlocks[col]));
//        }
        return originalBlocks;
    }

    public static List<Integer> primefactor(int zahl) {
